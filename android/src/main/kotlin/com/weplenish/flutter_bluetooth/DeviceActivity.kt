...
class DeviceActivity(private val channel: MethodChannel) : Activity() {
    companion object {
    ...
    fun connectToDevice(singleDevice: Boolean? = true, namePattern: String?, uuid: String?){
        val deviceFilter: BluetoothDeviceFilter = BluetoothDeviceFilter.Builder().let {
            when {
                namePattern != null ->
                    it.setNamePattern(Pattern.compile(namePattern))
                else -> it
            }
        }.let {
            when {
                uuid != null ->
                    it.addServiceUuid(ParcelUuid(UUID.fromString(uuid)), null)
                else -> it
            }
        }.build()
